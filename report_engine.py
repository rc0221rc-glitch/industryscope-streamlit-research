...
def kb_anchor_id(kb_url: str) -> str:
    chunk_id = kb_url.removeprefix("kb://").strip()
    safe = re.sub(r"[^A-Za-z0-9_.:-]+", "-", chunk_id).strip("-")
    return f"kb-source-{safe or 'chunk'}"


def rewrite_kb_links(rendered_html: str) -> str:
    def replace_anchor(match: re.Match[str]) -> str:
        url = html.unescape(match.group(1))
        anchor = kb_anchor_id(url)
        return f'<a href="#{anchor}" class="kb-link" title="跳转到报告内知识库证据片段"'

    rendered_html = re.sub(r'<a href="(kb://[^"]+)"', replace_anchor, rendered_html)

    def replace_plain(match: re.Match[str]) -> str:
        url = match.group(0)
        anchor = kb_anchor_id(url)
        escaped_url = html.escape(url)
        return f'<a href="#{anchor}" class="kb-link" title="跳转到报告内知识库证据片段">{escaped_url}</a>'

    parts = re.split(r"(<a\b[^>]*>.*?</a>)", rendered_html, flags=re.IGNORECASE | re.DOTALL)
    for idx, part in enumerate(parts):
        if idx % 2 == 0:
            parts[idx] = re.sub(r"(?<![\"'=])(kb://[A-Za-z0-9_.:-]+)", replace_plain, part)
    return "".join(parts)
...
def render_markdown_to_html(markdown_text: str) -> str:
    ...
    return rewrite_kb_links(rendered)


def render_kb_evidence_html(sources: list[dict[str, str]] | None) -> str:
    kb_sources = [
        source for source in (sources or [])
        if source.get("type") == "knowledge_base" or str(source.get("url", "")).startswith("kb://")
    ]
    if not kb_sources:
        return ""

    cards: list[str] = [
        '<section class="kb-evidence" id="kb-evidence">',
        "<h2>知识库证据片段</h2>",
        "<p>以下为本报告引用到的专属知识库片段。知识库内容可用于深度线索和私有资料佐证；涉及融资、订单、财务、政策和最新事实时仍需与公开一手来源交叉验证。</p>",
    ]
    seen: set[str] = set()
    for idx, source in enumerate(kb_sources, start=1):
        url = source.get("url", "")
...
def render_report_html(markdown_text: str, req: ReportRequest, sources: list[dict[str, str]] | None = None) -> str:
    ...
    return template.render(
        ...
        body_html=body_html,
        kb_evidence_html=render_kb_evidence_html(sources),
